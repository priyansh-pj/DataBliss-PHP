        ...
        <div class="card-body p-3">
          ...
          <div class="row align-items-center">
            ...
            <div class="alert alert-info col-8 mt-3" role="alert" style="opacity: 1; transition: opacity 0.5s ease-in-out;">
              ...
              <strong>Info!</strong> Enter Seller Contact No. If Seller exist data will be fetched  
              </span>
            </div>
            <div class="alert alert-info col-8 mt-3" role="alert" style="opacity: 1; transition: opacity 0.5s ease-in-out;">
              <span style="color:white;">
              ...
              <form class="form mt-1" action="<?= base_url('databliss/Billing/purchase_data')?>" method="post">
                <div class="row">
                <div class="col-md-6">  
                    <div class="form-group">
                      <label for="tel-input" class="form-control-label" style="font-size:1em;">Seller Contact No.<span style="color:red;"> * </span> : </label>
                      <input class="form-control" name="seller_phone" type="tel" placeholder="9999999999" id="seller_phone" required="required">
                    </div>
                  </div>
                  <div class="col-md-6">  
                    <div class="form-group">
                      <label for="text-input" class="form-control-label" style="font-size:1em;">Seller Name<span style="color:red;"> *</span> : </label>
                      <input class="form-control" name="seller_name" type="text" placeholder="Seller Name" id="seller_name" required="required">
                    </div>
                  </div>
                </div>
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                      <label for="text-input" class="form-control-label" style="font-size:1em;">Seller GSTIN : </label>
                      <input class="form-control" name="seller_gstin" type="text" placeholder="Seller GSTIN" id="seller_gstin">
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="form-group">
                      <label for="email-input" class="form-control-label" style="font-size:1em;">Seller Email : </label>
                      <input class="form-control" name="seller_email" type="email" placeholder="seller@mail.com" id="seller_email" required="required">
                    </div>
                  </div>
                </div>
                <div class="row">
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <input class="form-control" name="seller_city" type="text" placeholder="Seller Address" id="seller_city" required="required" readonly>
                    </div>
                  </div>
                  <div class="col-md-4">
                    <div class="form-group">
                      <label for="text-input" class="form-control-label" style="font-size:1em;">State<span style="color:red;"> * </span> :</label>
                      <input class="form-control" name="seller_state" type="text" placeholder="Seller State" id="seller_state" required="required" readonly>
                    </div>
                  </div>
                  <div class="col-md-4">
                    <div class="form-group">
                      <label for="number-input" class="form-control-label" style="font-size:1em;">Pincode<span style="color:red;"> * </span> :</label>
                      <input class="form-control" name="seller_pincode" type="number" placeholder="Seller Pincode" id="seller_pincode" required="required">
                    </div>
                  </div>
                </div>
                    ...
                    <table class="table align-items-center mb-0">
                      <thead>
                        <tr>
                          ...
                          <th class="text-center text-uppercase text-secondary text-sm font-weight-bolder " style="font-size:1em; width:8%;">SKU Code</th>
                      ...
                      <tr class = "items">
                        ...
                        <td style="text-align:center; padding-top: 0.8%;">
                          <div class="form-group">
                            <input class="form-control sku" type="text" value="0" id="sku" name="sku[]">
                          </div>
                        </td>
                        <td style="text-align:center; padding-top: 0.8%;">
                          <div class="form-group">
...
transportationCheckbox.addEventListener('change', (event) => {
  ...
  $(document).ready(function() {
    $('#seller_pincode').blur(function() { // When user loses focus from pincode input
        ...
        $.ajax({ // Make an AJAX call to fetch city and state from API
          ...
          success: function(result) {
            if (result[0].Status == 'Success') { // If API returns successful response
              ...
              $('#seller_city').val(city); // Update city field in the form
              $('#seller_state').val(state); // Update state field in the form
            }
          }
        });
      }
...
//  seller data fetch
$(document).ready(function(){
    $('#seller_phone').on('change', function(){
        var seller_phone = $(this).val();
        $.ajax({
            ...
            success: function(response){
                var data = response;
                $('#seller_name').val(data.contact_name);
                $('#seller_pincode').val(data.contact_pincode);
                $('#seller_email').val(data.contact_email);
                $('#seller_gstin').val(data.gst_number);
                var pincode = $('#seller_pincode').val(); // Get the pincode entered by the user
                if (pincode.length == 6) { // If pincode is valid
                    $.ajax({ // Make an AJAX call to fetch city and state from API
                        url: 'https://api.postalpincode.in/pincode/' + pincode,
                        type: 'GET',
...
$(document).ready(function() {
  ...
  $(document).on('change', '.product', function() {
    ...
    var $sku = $product.closest('tr').find('.sku');
