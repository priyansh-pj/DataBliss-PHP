      ...
      <div class="card">
        <div class="card-body p-3">
          ...
          <td style="text-align:center;" ><div class="col-12 ml-1">
              <a class="btn bg-gradient-secondary btn-lg" href="<?=base_url('databliss/Owner/edit_employee/'.$requests->employee_id)?>" type="submit">EDIT</a>
          </div> </td>
...
function getEmpId(event, employee_id) {
    ...
    $.ajax({
        url: <?= base_url("/databliss/Owner/edit_employee/")?> + employee_id,
        type: "GET",
        success: function(response) {
            alert("Employee terminated!");
            window.location.href = <?= (base_url('/databliss/Owner/access_employee_list')) ?>;
        },
